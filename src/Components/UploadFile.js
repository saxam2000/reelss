...
function UploadFile(props) {
    ...
    const onChange=(e)=>{
        ...
        console.log(file);
        if(!file){//check is file selected or not
            setError('Please select a file');
            setTimeout(()=>{setError(null)},2000)
            return;
        }

        if(types.indexOf(file.type)===-1)//check if it its a file or not
        {
        ...
        if(file.size/(1024*1024)>100)//check file is in under 100mb or not 
        {
            setError('The selected file is too big');
            setTimeout(()=>{setError(null)},2000)
            return;
        }
        const id = uuidv4();//gererate random id
        const uploadTask = storage.ref(`/posts/${props.userData.userId}/${file.name}`).put(file);
        uploadTask.on('state_changed',fn1,fn2,fn3);//attach listener 
